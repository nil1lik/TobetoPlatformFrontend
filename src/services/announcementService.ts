...
>{
  ...
  getAllAnnouncementTypeList(pageIndex: number=0, pageSize: number=3): Promise<AxiosResponse<GetAnnouncementTypeList, any>> {
		return axios.get<GetAnnouncementTypeList>(this.dtoUrl+`?PageIndex=${pageIndex}&PageSize=${pageSize}`);
	}
}
